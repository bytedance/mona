import { updateProps } from './processProps';
...
export default class ServerElement {
  ...
  serialize() {
    ...
    const { children: _, ...restProps } = this.props || {};
    // console.log('updateProps(restProps)', updateProps(restProps));
    const json: any = {
      key: this.key,
      type: this.type,
      text: this.text,
      props: updateProps(restProps),
      children: children,
    };
    return json;
  }
