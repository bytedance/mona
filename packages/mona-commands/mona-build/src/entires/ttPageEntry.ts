...
export const ttPageEntryMap = new Map<string, NativeComponentEntry>();

let id = 1;
// 将路径和jsx收集的prop一一对应
export const genNativeComponentId = (resourcePath: string) => {
  const entry = ttPageEntryMap.get(resourcePath);
  if (entry?.id) {
    return entry?.id;
  }
  return `ttPage${id++}`;
...
export const genNativeComponentEntry = (configHelper: ConfigHelper, entryPath: string) => {
  if (ttPageEntryMap.has(entryPath)) {
    return ttPageEntryMap.get(entryPath)!;
  } else {
    const nEntry = new NativeComponentEntry(configHelper, entryPath);
    ttPageEntryMap.set(entryPath, nEntry);
    return nEntry;
  }
};
